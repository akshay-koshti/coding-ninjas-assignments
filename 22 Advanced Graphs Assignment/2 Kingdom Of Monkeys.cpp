...
void dfs(vector<vector<int>> &graph, vector<long long> &bananas, int start, long long &count){
	visited[start] = 1;
	count += bananas[start];
		for(int i=0; i<graph[start].size(); i++){
		int next = graph[start][i];
		if(!visited[next])
			dfs(graph, bananas, next, count);
	}
}

long long solve(vector<vector<int>> &graph, vector<long long> &bananas, int n){
...
int main(){
	...
	while(T--){
		...
		for(int i=0; i<n; i++)
			cin >> bananas[i];
		cout << solve(graph, bananas, n) << endl;
	}
	return 0;
}