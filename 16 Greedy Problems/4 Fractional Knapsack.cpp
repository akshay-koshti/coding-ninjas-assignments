...
long long int amount=arr[0].cost;
for(int i=1;i<n;i++)
{
if (arr[i].speed > arr[0].speed)
{
d = d-((arr[i].time-arr[0].time)*arr[0].speed);
if (d<=0)
break;
amount = amount + arr[i].cost;
arr[0].speed = arr[i].speed;
arr[0].time= arr[i].time;
}}
cout<<amount;
return 0;
}