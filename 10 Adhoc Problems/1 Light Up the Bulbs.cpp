...
long long cost(long long bulbs[], long long n, long long x, long long y){
long long groups=0, ans;
if(bulbs[0] == 0) groups++;
for(long long i=0; i<n-1; i++)
if(bulbs[i] && (!bulbs[i+1])) groups++;
...
string s;
cin >> s;
long long *bulbs=new long long[n];
for(long long i=0; i<n; i++)
bulbs[i] = s[i]-'0';
cout << cost(bulbs, n, x, y);
delete[] bulbs;
return 0;
}