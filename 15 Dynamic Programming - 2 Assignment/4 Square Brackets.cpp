...
int current_index = open+close+1;
if(open == n && close == n){
dp[open][close] = 1;
}else if(open == close || openings[current_index]){
dp[open][close] = solve(open+1, close, n);
}else if(open == n){
dp[open][close] = solve(open, close+1, n);
}else{
dp[open][close] = solve(open+1, close, n) + solve(open, close+1, n);
}
...
int n, k;
cin >> n >> k;
openings = vector<bool>(40);
dp = vector< vector<int> >(40, vector<int>(40, -1));
for(int i=0; i<k; i++){
int temp;
cin >> temp;
openings[temp] = true;
